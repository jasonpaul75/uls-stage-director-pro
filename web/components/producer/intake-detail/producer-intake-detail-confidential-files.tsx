...
export function ProducerIntakeConfidentialFilesSection(props: {
  ...
  return (
    <ProducerIntakeSectionShell
      id="uls-confidential-files"
      title="Confidential files (ULS only)"
      description={
        <p>
          Store agreement PDFs and insurance/compliance artifacts ULS controls. Directors never see this block — downloads are
          production-authenticated only. DocuSign remains the signing system of record; this is separate document retention in your
          bucket.
        </p>
      }
    >
      {!s3 ? (
        <p className="rounded-md border border-amber-900/55 bg-amber-950/25 px-3 py-2 text-[11px] leading-relaxed text-amber-100">
      ...
      {attachments.length > 0 ? (
        <ul className="space-y-2 text-xs text-uls-muted">
          {attachments.map((a) => (
            <li
              key={a.id}
              className="flex flex-col gap-2 rounded-uls-card border border-uls-border bg-uls-surface/50 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"
            >
              <div className="min-w-0">
                <p className="truncate font-medium text-uls-text">{a.fileName}</p>
                <p className="text-[10px] text-uls-subtle">
                  {kindLabel(a.kind)} · {formatKb(a.sizeBytes)} · {(a.uploadedBy.name ?? "").trim() || a.uploadedBy.email} ·{" "}
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                ...
                <form action={deleteProjectAttachment}>
                  ...
                  <button type="submit" className="text-[11px] text-red-400/90 underline-offset-2 hover:text-red-300 hover:underline">
                    Remove
                  </button>
                </form>
              </div>
        ...
        <p className="text-xs text-uls-subtle">No confidential uploads yet.</p>
      )}

      <form action={uploadProjectAttachment} className="flex flex-col gap-3">
        <input type="hidden" name="projectId" value={projectId} />
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-uls-muted">Classification</span>
          <select name="kind" required className={producerIntakeFieldClass}>
            <option value="CONTRACT">Contract / agreement file</option>
            <option value="INSURANCE_COMPLIANCE">Insurance &amp; compliance</option>
          </select>
        </label>
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-uls-muted">File</span>
          <input
            name="file"
            type="file"
            required
            ...
            disabled={!s3}
            className="text-xs text-uls-muted file:mr-3 file:rounded file:border file:border-uls-border file:bg-uls-surface-inset file:px-2 file:py-1 file:text-uls-text disabled:opacity-40"
          />
        </label>
        <Button
          type="submit"
          ...
          size="sm"
          disabled={!s3}
          className="w-fit border-amber-800/70 bg-amber-950/30 text-amber-200 hover:bg-amber-900/40 disabled:cursor-not-allowed disabled:opacity-40"
        >
